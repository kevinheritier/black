import numpy as np
from exceptions import EmptyAssetListError

from analytics.portfolio_types import Covariances, CovariancesReturns, Portfolio, WeightedAsset
...
def optimize_weights(assets: list[AssetId], covariant_returns: CovariancesReturns, kappa: Kappa) -> Portfolio:
    ...
    sorted_weights = np.linalg.solve(
        kappa * covariant_returns.covariances.to_numpy(assets),
        np.array([covariant_returns.returns[asset] for asset in assets]),
    )
    sorted_weights /= sum(sorted_weights)
    return Portfolio([WeightedAsset(asset, w) for asset, w in zip(assets, sorted_weights, strict=False)])
...
def imply_returns_from_optimal_weights(portfolio: Portfolio, covs: Covariances, kappa: Kappa) -> Returns:
    ...
    return Returns(dict(zip(assets, returns, strict=False)))
