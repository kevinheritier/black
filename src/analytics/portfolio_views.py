...
#             logging.error("Invalid type: data must be either dataframe or dict with name of assets as keys")
...
#                 self._df = self._df.transpose()
#             else:
#                 self._df.loc["w", :] = 1.0 / self._df.shape[1]
...
#                 logging.error("Covariance dataframe {} must be symmetric with same index and columns names".format(cov))
#             if (cov.index != cov.columns).all():
#                 logging.error("Covariance indices: {} must be equal to covariance columns: {}").format(
#                     cov.index, cov.columns
#                 )
#             if not cov.equals(cov.transpose()):
#                 logging.error("Covariance dataframe {} must be symmetric".format(cov))
#             self._cov = cov

#         else:
#             print("Assuming no correlation between assets, covariance matrix = I")
#             self._cov = pd.DataFrame(np.eye(self._df.shape[1]), columns=self.df.columns, index=self.df.index)
...
#         df = df.rename(index={r_name: "r", w_name: "w"})
#         self.df = df
...
#         """
#         if inplace:
#             self._df.loc["r", :] = self.kappa * np.dot(self._cov.values, self.w.values)
#         else:
...
#         Z = self.r + np.dot(tau * np.dot(self.cov, V.P.T), np.dot(PSPt, X)).reshape(V.P.shape[1])
#         copy = self.copy()
#         copy.r = Z
#         copy.optim_w()
#         return copy.r
...
#         if 0.0 in df["c"]:
#             logging.warning(
#                 """Dropping views with 0% confidence:
#             {}""".format(df[df["c"] == 0.0])
#             )
#             df = df[df["c"] != 0.0]
...
#             for coeff, r, c in zip(
#                 df_dup.drop(["r", "c"], axis=1).to_dict("records"),
#                 df_dup["r"],
#                 df_dup["c"],
#             ):
...
#         return View(
#             P=self._df.drop(["r", "c"], axis=1).loc[i, :],
#             r=self._df.loc[i, "r"],
#             c=self._df.loc[i, "c"],
#         )
...
#         """
#         s = " ".join(["{:.1%} {}".format(coeff, name) for (name, coeff) in zip(coeff_dict.keys(), coeff_dict.values())])
...
#         df = df.rename(columns={r_name: "r", c_name: "c"})
#         self.add_views(df)
...
#         # inv(pk tau Sigma pk')
#         PkPiPk_1 = 1.0 / view_k.P.dot(tau * self.portfolio.cov.dot(view_k.P))
#         r_100 = self.portfolio.r + tau * self.portfolio.cov.dot(view_k.P) * PkPiPk_1 * X
#         if inplace:
...
#         first_term = np.linalg.inv(InvSig + np.outer(view_k.P, view_k.P) / omega)
#         second_term = np.dot(InvSig, pproblem.portfolio.r) + view_k.P * view_k.r / omega
#         new_cov = pproblem.portfolio.cov  # c'est le sigma "star" ?
#         w_k = np.linalg.solve(pproblem.portfolio.kappa * new_cov, np.dot(first_term, second_term))
...
#             [PortfolioProblem.f_k(omega, pproblem, view, tau=tau) for omega, view in zip(omegas, pproblem.views)]
#         )
...
#         logging.info(
#             """Setting up optimization for portfolio:
#             {}
#             and views:
#             {}""".format(self.portfolio.df, self.views)
#         )
#         result = minimize(PortfolioProblem.f, omegas_0, args=(self, tau), method="Nelder-Mead")
#         if result.success:
...
#             logging.error("Confidence in views: {} must be strictly between 0 and 1".format(self.views.df.c.values))
#         alphas = np.array([(1.0 - view.c) / (view.c) for view in self.views])
#         omegas_star = np.array(
#             [tau * alpha * (view.P @ self.portfolio.cov).dot(view.P) for alpha, view in zip(alphas, self.views)]
#         )
#         return omegas_star
...
#         if not alt:
#             if any(omegas == 0):
#                 logging.error("omegas must be strictly different than zero: {}".format(omegas))
...
#         self.portfolio = ptf
#         views = Views()
#         self.views = views.read_xlsx_views(file_path, r_name="R", c_name="C", sheet_name=views_sheet)
...
#         """
#         Sigma = self.m_portfolio._cov
#         first_term = self.t_portfolio.df.loc["r"].values - self.m_portfolio.df.loc["r"].values
#         second_term = np.linalg.inv((tau * Sigma) @ np.linalg.inv(tau * Sigma + self.Omega))
